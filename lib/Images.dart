...
class Images{
  ...
  List<File> sortFilesByDate(String date) {
    var filesSortByDate ;
    filesSortByDate = filesAll.where((item) => item.toString().contains('$date')).toList();
  ...
  List<String> getYearFromFiles(){
    ...
    for(int i =0; i < filesAll.length; i++){
      yearFromFilename.add(filesAll[i].toString().split('/')[4].substring(0, 4));
    }
  ...
  List<String> getMonthFromFiles(){
    ...
    for(int i =0; i < filesAll.length; i++){
      monthFromFilename.add(filesAll[i].toString().split('/')[4].substring(0, 6));
    }
  ...
  List<String> getDateFromFiles(){
    ...
    for(int i =0; i < filesAll.length; i++){
      dateFromFilename.add(filesAll[i].toString().split('/')[4].substring(0, 8));
    }
